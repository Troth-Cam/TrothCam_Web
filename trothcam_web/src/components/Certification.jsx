...
const Certification = () => {
    ...
    //좋아요 누르기
    const HeartImg = ({ src , productId, setProductDetail}) => {
        
        //console.log("access토큰 확인",accessToken);
        const handleLikeToggle = async () => {
            
    
            try {
                let response1;
                console.log(detail.liked);
                console.log(productId);
                if (detail.liked) {
                    response1 = await axios.delete(`/api/like-product/${productId}`,  {headers: {"Authorization" : `Bearer ${accessToken}`}});
                } else {
                    console.log("좋아요 누르기전")
                    response1 = await axios.post(`/api/like-product/${productId}`, {headers: {"Authorization" : `Bearer ${accessToken}`}});
                    console.log("좋아요 누른 후")
                }
            ...
            <Col>
                ...
                <TextContainer>
                        ...
                        <Title2>#{detail.tags}</Title2>
                            ...
                            <HeartImg src={detail.liked ? redheartIcon : blackheartIcon} productId={productId}
